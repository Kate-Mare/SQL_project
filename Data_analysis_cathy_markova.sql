...
distinct food_code,
food_category
from t_cathy_markova_project_sql_primary_final tmpf
where food_category in ('Chléb konzumní kmínový', 'Mléko polotučné pasterované') -- 111301 - chléb, 114201 - mléko 
 ;*/
...
SELECT
    ...
    ROUND(AVG(tmpf.average_wages) :: numeric, 2) AS avg_annual_wage,
    yd.avg_annual_food_price,
    FLOOR(AVG(tmpf.average_wages) / yd.avg_annual_food_price) AS purchasable_quantity
...
JOIN t_cathy_markova_project_sql_primary_final tmpf
    ON CAST(yd.year AS INTEGER) = EXTRACT(YEAR FROM tmpf.price_measured_from :: DATE)  
